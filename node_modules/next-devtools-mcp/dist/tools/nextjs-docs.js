...
export const inputSchema = {
    topic: z
        .string()
        .optional()
        .describe("Optional: what you're looking for (e.g. 'use cache', 'generateMetadata', 'middleware'). Used only to suggest where to look in the bundled docs."),
    project_path: z
        .string()
        .optional()
...
function parseMajor(versionish) {
    if (!versionish)
        return null;
...
function resolveNextVersion(projectPath) {
    try {
        const installedPkg = path.join(projectPath, "node_modules", "next", "package.json");
        if (fs.existsSync(installedPkg)) {
            const { version } = JSON.parse(fs.readFileSync(installedPkg, "utf8"));
            if (typeof version === "string")
                return { version, source: "installed" };
        }
    }
    catch {
...
export async function handler({ topic, project_path }) {
